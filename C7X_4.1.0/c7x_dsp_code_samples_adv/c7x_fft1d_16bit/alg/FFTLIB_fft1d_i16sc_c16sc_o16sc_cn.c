...
int16_t crot90HIm(int16_t re, int16_t im)
{
    return re;
}
...
void shiftRightAndRound(int16_t * pBuff, uint32_t numElements, uint32_t shift)
{
    ...
    for (k = 0; k < numElements; k++) {
        pBuff[k] = (pBuff[k] & shiftOutBitMask) != 0 ?
                   (pBuff[k] >> shift) + 1 : (pBuff[k] >> shift);
                                                ...
                                                FFTLIB_bufParams1D_t * bufParamsShift)
{
    ...
    status = FFTLIB_fft1d_i16sc_c16sc_o16sc_checkParams_cn(pX, bufParamsX,
                                                           pW, bufParamsW,
                                                           pY, bufParamsY,
...
#endif
    {
        ...
        while (numPointsPerDft > 2) {
            ...
            for (k = 0; k < numPoints/4; k++) {
                ...
                if ((numPointsPerDft == 4) || ((numPointsPerDft == 8) && (dftPointIndex == 0)) ) {
                /* For this case, the twiddle factors are all 1's. In this (fixed point) variant,
                 * multiplication by 1 and scaling down can introduce errors. So, like in the
                 * kernel implementation, we skip the multiplication by 1's                       */
                    butterflyNoTw(pX[dftStartOffset*2 + 2*dftPointIndex],
                                  pX[dftStartOffset*2 + 2*dftPointIndex + 1],
                                  pX[dftStartOffset*2 + (numPointsPerDft/4)*2 + 2*dftPointIndex],
                                  pX[dftStartOffset*2 + (numPointsPerDft/4)*2 + 2*dftPointIndex + 1],
                                  pX[dftStartOffset*2 + (numPointsPerDft/2)*2 + 2*dftPointIndex],
                                  pX[dftStartOffset*2 + (numPointsPerDft/2)*2 + 2*dftPointIndex + 1],
                                  pX[dftStartOffset*2 + 3*(numPointsPerDft/4)*2 + 2*dftPointIndex],
                                  pX[dftStartOffset*2 + 3*(numPointsPerDft/4)*2 + 2*dftPointIndex + 1],
                                  bflyOut);
                }
                else {
                    butterfly(pX[dftStartOffset*2 + 2*dftPointIndex],
                              pX[dftStartOffset*2 + 2*dftPointIndex + 1],
                              pX[dftStartOffset*2 + (numPointsPerDft/4)*2 + 2*dftPointIndex],
                              pX[dftStartOffset*2 + (numPointsPerDft/4)*2 + 2*dftPointIndex + 1],
                              pX[dftStartOffset*2 + (numPointsPerDft/2)*2 + 2*dftPointIndex],
                 ...
                 * stage combined.                                                   */
                if (numPointsPerDft != 8) shiftRightAndRound(bflyOut, 8, shift);

                pX[dftStartOffset*2 + 2*dftPointIndex] = bflyOut[0];
                pX[dftStartOffset*2 + 2*dftPointIndex + 1] = bflyOut[1];
                pX[dftStartOffset*2 + (numPointsPerDft/4)*2 + 2*dftPointIndex] = bflyOut[4];
                pX[dftStartOffset*2 + (numPointsPerDft/4)*2 + 2*dftPointIndex + 1] = bflyOut[5];
                pX[dftStartOffset*2 + (numPointsPerDft/2)*2 + 2*dftPointIndex] = bflyOut[2];
                pX[dftStartOffset*2 + (numPointsPerDft/2)*2 + 2*dftPointIndex + 1] = bflyOut[3];
                pX[dftStartOffset*2 + 3*(numPointsPerDft/4)*2 + 2*dftPointIndex] = bflyOut[6];
                pX[dftStartOffset*2 + 3*(numPointsPerDft/4)*2 + 2*dftPointIndex + 1] = bflyOut[7];
            }
            numPointsPerDft /= 4;
