...
#pragma DATA_SECTION(residual, ".residual");
ALIGN_128BYTES float residual[MD_4x8];

#pragma DATA_SECTION(pBlock, ".pBlock")
ALIGN_128BYTES uint8_t pBlock[VXLIB_KALMANFILTER_4X8_CORRECT_F32_PBLOCK_SIZE];
...
int test_main(uint32_t *pProfile)
{
    ...
    result  = 1;
      ...
      VXLIB_kalmanFilter_4x8_Correct_F32_init(pKF, &Z[0], &residual[0], &pBlock);

      //Load the predicted state and covariance and measurement
       for(ctr = 0; ctr < SD_4x8; ctr++)
       {
           pKF->predictedState[ctr] = predState[0][ctr];
       }

       for(ctr = 0; ctr < (SD_4x8 * SD_4x8); ctr++)
       {
           pKF->predictedErrorCov[ctr] = predCovariance[0][ctr];
       }

       for(ctr = 0; ctr < MD_4x8; ctr++)
       {
           Z[ctr] = measurement[0][ctr];
       }
      ...
      t0 = PROFILE_READ;
       ...
       for(ctr = 0; ctr < NUM_OBJECTS; ctr++) {
           ...
           VXLIB_kalmanFilter_4x8_Correct_F32_kernel(pKF, &Z[0], &residual[0], &pBlock);
       }
    ...
    printf("Comparing results! \n");
       ...
       for(ctr = 0; ctr < SD_4x8; ctr++)
       {
          if(ABS(pKF->state[ctr] - correctedState[0][ctr]) > 0.0001)
          {
#if(ERROR_REPORT)
             printf("State Mismatch at ctr = %d, pOut = %f, pOutRef = %f, diff = %f \n", ctr, pKF->state[ctr], correctedState[0][ctr], (pKF->state[ctr] - correctedState[0][ctr]));
#endif
             result = 0;
          }
      }
       ...
       for(ctr = 0; ctr < (SD_4x8 * SD_4x8); ctr++)
       {
          if(ABS(pKF->errorCov[ctr] - correctedCovariance[0][ctr]) > 0.001)
          {
#if(ERROR_REPORT)
             printf("Covariance Mismatch at ctr = %d, pOut = %f, pOutRef = %f, diff = %f \n", ctr,
                      pKF->errorCov[ctr], correctedCovariance[0][ctr], (pKF->errorCov[ctr] - correctedCovariance[0][ctr]));
#endif
             result = 0;
          }
      }
